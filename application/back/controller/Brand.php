<?php
/**
 * Created by PhpStorm.
 * User: lenovo
 * Date: 2017/8/28
 * Time: 19:55
 */

namespace app\back\controller;
use app\back\validate\BrandCreate;
use app\back\validate\BrandSet;
use think\Config;
use think\Controller;
use think\Request;
use app\back\model\Brand as BrandModel;
use think\Session;
use think\Validate;

class Brand extends Controller
{


    /**
     * 设置方法(包括添加,修改两个功能)
     */
    public function set($id=null)
    {
        #   实例化请求对象
        $request = Request::instance();
        #   get提交
        // get 请求, 展示表单form
        if ($request->isGet()) {
            // 品牌数据, 可能没有
            $row = BrandModel::get($id);

            // 可能存在错误消息和错误数据
            return $this->fetch('set', [
                // id也分配过去
                'id' => $id,
                // 分配的数据, 从session中获取, 并设置默认值
                'errorInfo' => Session::get('errorInfo') ?: [],
                // 数据, 可能是错误数据或编辑数据, 错误数据优先
                'data' => Session::get('errorData') ?: $row
            ]);
        }
        #   post提交
        elseif ($request->isPost()) {
            //1.接受数据
//          $data = $request->post();   //请求对象的方式
            $data = input('post.');


            //判断时候有文件上传
            if(isset($data['is_delete'])){
                $this->deleteLogo($id);
                $data['logo'] = '';
            }else{

                if($logo = $request->file('logo')) {
                    $upload_path = ROOT_PATH . '/public/uploads/';
                    $file = $logo->validate(['size' => 1 * 1024 * 1024, 'ext' => ['jpg', 'gif', 'png'], 'type' => ['image/png', 'image/jpg', 'image/gif', 'image/jpeg']])
                        ->move($upload_path);
                    if ($file) {
                        //上传成功,记录存储位置
                        $file_name = $file->getSaveName();
                        $data['logo'] = $file_name;
                    }
                    if(!is_null($id)){
                        //删除原来的图片
                        $this->deleteLogo($id);
                    }
                }
            }


            //2.校验数据
            $volidate = new BrandSet();
            if(!$volidate->batch(true)->check($data)){
                //校验未通过
                return $this->redirect('set',['id'=>$id],304,[
                    'errorInfo' =>  $volidate->getError(),
                    'errorData' =>  $data
                ]);
            }

            //3.添加或修改数据
            if (empty($id)) {
                // 新模型对象
                $model = new BrandModel();
            } else {
                $model = BrandModel::get($id);
            }

            // 执行返回受影响的记录数
            $affectedRows = $model
                // 允许自动赋值的字段
                ->allowField([
                    'title', 'site', 'logo', 'sort', 'create_time', 'update_time'
                ])
                ->data($data)
                ->save();

            if ($affectedRows !== false) {
                # 4. 重定向到index动作
                return $this->redirect('index');
            } else {
                // 插入错误
                return $this->redirect('create');
            }
        }
    }


    /**
     * 品牌展示
     */
    public function index()
    {
        #   1.搜索
        $model = new BrandModel();
        $pageQuery = [];
        //获取关键字
        $filter_title = input('filter_title',null);
        if (!is_null($filter_title)){
            $model->where('title','like',$filter_title."%");
            //记录搜索的记录,以便分页链接使用
            $pageQuery['filter_title'] = $filter_title;
        }
        //分配反显的搜索关键字
        $this->assign('filter_title',$filter_title);

        #   2.排序
        $orderStr = '';
        $order_field = input('order_field',null);
        $order_type = input('order_type','asc');
        if(!is_null($order_field)){
            $orderStr = $order_field." ".$order_type;
        }
        $model->order($orderStr);
        //将排序的字段和方式分配到模板
        $this->assign('order_field', $order_field);
        $this->assign('order_type',$order_type);


        #   3.分页
        //查询数据
        $limit = 2;
        //给分页链接添加关键字参数传递
        $data = $model->paginate($limit,false,[
            'query' =>  $pageQuery
        ]);
        return $this->fetch('index',[
            'rows'  =>  $data
        ]);
    }

    /**
     * 验证title是否唯一
     */
    public function checkTitleUnique()
    {
        //使用验证:必须返回一个字符串的true或false,才可以正常使用.
        //return true;
        Config::set('default_ajax_return','html');
        return Validate::unique(null,'brand,title,'.input('id'),[
            'title'=>input('title'),
            'id'=>input('id')
        ],'title')?'true':'false';
    }

    /**
     * 删除原来的logo图片
     */
    public function deleteLogo($id){
        //查询出原来的图片路径
        $oldFileName = (new BrandModel())->where('id',$id)->value('logo');
        if($oldFileName){
            @unlink(ROOT_PATH.'/public/uploads/'.$oldFileName);
        }
        return $this;
    }

    /**
     * 批量删除
     */
    public function batch()
    {
        #   接收数据
        //注意:用input接收数组数据是,必须加/a;
        //并且这里不建议使用默认值,空数组会导致全部删除,或修改destroy
        $ids = input('selected/a',[]);
        //dump($ids);die;

        //删除图片对应图片信息
        foreach($ids as $id){
            $this->deleteLogo($id);
        }

        BrandModel::destroy($ids);
        return $this->redirect('index');
    }

}